...
class BuildPartnerBase(BaseModel):
    ...
    description: Optional[str] = Field(..., description="Build Partners's details.", example="Pikachu is a good match with Inmortal Snorlax because it could damage the enemies while Snorlax is receiving all the enemies' attacks.")
...
class BuildBase(BaseModel):    
    ...
    description: str = Field(..., description="Build's details.", example="Inmortal Snorlax is a build that gives Snorlax extra HPs to make Snorlax's life inexhaustible.")
    ...
    first_movement: int = Field(..., description="Identifier of the move that should be taken when the battle starts.", example=2)
...
class Build(BuildBase):
    ...
    build_partners: Optional[List[BuildPartner]]
